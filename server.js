...
function runScenario(name, res) {
  ...
  switch (name) {
    ...
    case "duplicate-event": {
      emit("call.ended", { call_id: callId, crm_contact_id: contactId });
      const ev = emit("disposition.selected", { call_id: callId, finish_code: "CONSULTATION_BOOKED", agent_note: "booked" });
      deliverWebhook({ ...ev, __forceRedeliver: true });
      break;
    }
    case "conflict-409":
      emit("call.ended", { call_id: callId, crm_contact_id: contactId });
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (path.match(/^\/crm\/objects\/deals\/[^/]+$/) && method === "PATCH") {
      ...
      if (!deal) return sendJSON(res, 404, { error: "not found" });
      const body = await readBody(req);
      if (body.expected_lastmodifieddate && body.expected_lastmodifieddate !== deal.lastmodifieddate) {
        return sendJSON(res, 409, { error: "conflict", current: deal });
      }
      if (!body.ccs_changed_by || !body.ccs_change_reason) {
        return sendJSON(res, 400, { error: "ccs_changed_by ve ccs_change_reason zorunlu (denetim şartı)" });
      }
      const result = patchDeal(deal.id, body);
      return sendJSON(res, result.status, result.deal || deal);
    }
